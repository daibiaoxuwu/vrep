...
function sysCall_actuation() 
    ...
    elseif(maxCorr<0.7 and maxCorr > 0.5)then
        deltaSpeed = 0
    elseif(maxCorr<0.5 and maxCorr>-1 and math.abs(euler[1]) < 0.01 and math.abs(euler[2]) < 0.01)then
        deltaSpeed = -0.1
    elseif(maxCorr<-1)then
        deltaSpeed = 0
    end
...
--    deltaSpeed = 0.1*(desSpeed + linearSpeed[2])
    ...
    alphaCorr=0.00323 + (euler[1]+deltaSpeed)*0.225 + 1.4*(euler[1]-pAlphaE)-- + 0.005 * alphaCumul
    pAlphaE=euler[1]
    ...
    betaCorr=(euler[2] + deltaSpeed)*0.225 + 1.4*(euler[2]-pBetaE)-- + 0.001 * betaCumul
    pBetaE=euler[2]
