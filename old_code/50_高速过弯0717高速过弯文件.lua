...
function sysCall_init() 
    ...
    door[2]=sim.getObjectPosition(sim.getObjectHandle('GateCounter_80cmX190cm#0'),-1)
    door[3]=sim.getObjectPosition(sim.getObjectHandle('GateCounter_80cmX190cm#1'),-1)
    door[4]=sim.getObjectPosition(sim.getObjectHandle('GateCounter_80cmX190cm#2'),-1)
    door[5]=sim.getObjectPosition(sim.getObjectHandle('GateCounter_55cmX40cm#0'),-1)
    door[6]=sim.getObjectPosition(sim.getObjectHandle('GateCounter_55cmX40cm#1'),-1)
    door[7]=sim.getObjectPosition(sim.getObjectHandle('GateCounter_80cmX190cm'),-1)
...
function sysCall_actuation() 
    ...
    elseif(ctrl_flag==1) then
        ...
        if(target_pos[1] ~= target_platform_pos[1] or target_pos[2] ~= target_platform_pos[2] ) then
            ...
            speedx = math.abs(target_pos[1]-pos[1])*1
            speedy = math.abs(target_pos[2]-pos[2])*1
    ...
    elseif(ctrl_flag==4) then
        ...
        ctrl_flag_last_time = ctrl_flag_last_time - 1
        if(pos[3] <0.44) then ctrl_flag=5 end
    ...
    elseif(ctrl_flag==7) then
      ...
      --  pos_to_d = sim.multiplyVector(m,linearSpeed_resolute)
        ...
        print(speed_to_door, pos_to_door, entry_pos_to_door)
        if(ctrl7_cnt == 0)then
            --moving infrontof door
            ...
            --if arrived
            if(math.abs(pos_to_door[1] - entry_pos_to_door[1]) < 0.5 and math.abs(pos_to_door[2] - entry_pos_to_door[2]) < maxDistX and math.abs(speed_to_door[1]) < maxSpeedX) then
                ctrl7_cnt = 1
            end
        elseif(ctrl7_cnt == 1)then
            --moving through door
            way = normalization({pos[1]-door_pos[1], pos[2]-door_pos[2]},2)                
            
            target_pos[1] = pos[1]-way[1]-linearSpeed[1]/2
            target_pos[2] = pos[2]-way[2]-linearSpeed[2]/2

            --if through
            if(math.abs(pos_to_door[1]) < 0.2) then
                ctrl7_cnt = 2
            end
        elseif(ctrl7_cnt == 2)then
            --leave the door
            target_pos[1] = leave_pos[1]
            target_pos[2] = leave_pos[2]
