    ...
    elseif(ctrl_flag==7) then
      ...
      --  pos_to_d = sim.multiplyVector(m,linearSpeed_resolute)
        ...
        if(ctrl7_cnt == 0)then
            ...
            --if arrived
            if(pos_to_door[1] > entry_pos_to_door[1] - 0.5 and math.abs(pos_to_door[2] - entry_pos_to_door[2]) < maxDistX and math.abs(speed_to_door[1]) < maxSpeedX) then
                ctrl7_cnt = 1
            end
        elseif(ctrl7_cnt == 1)then
            --moving through door
            way = normalization({pos[1]-door_pos[1], pos[2]-door_pos[2]},1)                
            target_pos[1] = door_pos[1]-way[1]
            target_pos[2] = door_pos[2]-way[2]
