function sysCall_init() 
    ...
    ------------------ Obstacles position -------------------------------------
    target_platform = sim.getObjectHandle('Car')
...
function sysCall_actuation() 
    ...
    if(ctrl_flag==0) then
        ...
        print('totalforce:'+math.sqrt(math.pow(total_force[1],2)+math.pow(total_force[2],2)))
    ...
    elseif(ctrl_flag==1) then
        ...
        if(target_distance < ctrl0_target_distance * 0.5)then ctrl_flag=2 end
    ...
    elseif(ctrl_flag==2) then
        ...
        if(linearSpeedTotal < 2)then ctrl_flag = 3 end
