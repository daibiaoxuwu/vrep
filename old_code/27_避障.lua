...
function sysCall_actuation() 
    ...
    target_platform_pos = sim.getObjectPosition(target_platform, -1)
    target_platform_pos[1] = target_platform_pos[1] + target_diff[1] 
    target_platform_pos[2] = target_platform_pos[2] + target_diff[2] 
    ...
    elseif(ctrl_flag==6 or ctrl_flag == 1)then
        if(ctrl_flag==6)then 
            target_height = 1
        ...
        if(linearSpeedTotal < speed)then
            ...
            if(target_distance < -1)then
                ctrl_angle_x = linearSpeed[1] * ctrl_angle_max / 10
                ctrl_angle_y = linearSpeed[2] * ctrl_angle_max / 10
    ...
    if(ctrl_flag==1)then
        if(target_distance < 0.02 and linearSpeedTotal < 0.13 and euler[3] < math.rad(60) and euler[3]>math.rad(-60))then
            print("-------------!!!!!!!!!!!!1------------")
    ...
    elseif(ctrl_flag==6)then
        if(target_distance < 0.3 and linearSpeedTotal < 0.3 )then
            print("-------------!!!!!!!!!!!!1------------")
            ctrl_flag = ctrl_flag + 1
            target_height=1.5
        end
        ...
        if(target_distance < 0.3 and linearSpeedTotal < 0.3 )then
            print("-------------!!!!!!!!!!!!1------------")
            ctrl_flag = ctrl_flag + 1
            target_platform = sim.getObjectHandle('GateCounter_55cmX40cm')
        end

    elseif(ctrl_flag==2) then
        sim.setJointTargetPosition(revolute0,math.rad(-40))
        sim.setJointTargetPosition(revolute1,math.rad(-40)) 
        target_height = 0
        ctrl_flag_last_time=15
        ctrl_flag=3
    elseif(ctrl_flag==3) then
        ctrl_flag_last_time = ctrl_flag_last_time - 1
        if(pos[3] <0.45) then ctrl_flag=4 end
    ...
    elseif(ctrl_flag==5)then
        ...
        ctrl_flag_last_time = ctrl_flag_last_time - 1
        if(ctrl_flag_last_time <0) then ctrl_flag=6
        end
