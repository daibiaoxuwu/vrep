...
function sysCall_actuation() 
    ...
    if(path_ctrl_flag == 0)then
        ...
        for i=1,6,1 do  
            newPoint={0,0,0}
            if(CircleLineCross(tree[i][1], tree[i][2], 3, pos[1], pos[2], target_platform_pos[1], target_platform_pos[2], newPoint)) then
                print(newPoint)
            end               
        end
    elseif(path_ctrl_flag == 2)then
    end
    ...
    elseif(ctrl_flag==1 or ctrl_flag==7) then
        ...
        angle = math.max(0.1, math.min(1.5,target_distance / 4))
        speed = math.min(10, target_distance)
    ...
    elseif(ctrl_flag==2 or ctrl_flag==3) then
        ...
        if(math.abs(euler[1]) < 0.07 and math.abs(euler[2]) < 0.07 and ctrl_flag_last_time < 0) then
            ...
            if(minx < 10000 and miny < 10000)then
                ...
                max_descend_error = target_height * 0.15

                descend_error = math.sqrt(9 * math.pow(pos[1] - target_platform_pos[1], 2) + math.pow(pos[2] - target_platform_pos[2], 2))
