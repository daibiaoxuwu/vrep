...
function sysCall_actuation() 
 ...
 angle = 0.5
    ...
    elseif(ctrl_flag==2 or ctrl_flag==3 or ctrl_flag==8) then
        ...
        if(ctrl_flag == 8)then max_descend_error = max_descend_error * 2 end
        --1.5: 0.675 0.5:0.025
        ...
        if(math.abs(euler[1]) < maxEuler and math.abs(euler[2]) < maxEuler and ctrl_flag_last_time < 0) then
            ...
            for i=1,xlen,2 do  
                for j=1,ylen,30 do
                    if(not((j>1800 and j<1900 and i>30 and i<60) or (j>2500 and j<2600 and i>470 and i<490))) then
                    
                        if (imageBuffer[i*ylen+j]>0.8 and 
                            ((ctrl_flag ~= 3 and imageBuffer[i*ylen+j+1]>0.8 and imageBuffer[i*ylen+j+2]>0.8) 
            ...
            if(minx < 10000 and miny < 10000)then
                ...
                if(ctrl_flag ~= 3) then
                    if(maxx > 720-7)then
                        maxx = minx + math.max(560 / (pos[3]-target_platform_height), (maxy - miny)/3)
                    end
                    if(minx < 7) then
                        minx = maxx - math.max(560 / (pos[3]-target_platform_height), (maxy - miny)/3)
                    end
                    if(maxy > 3840-38)then
                        maxy = miny + 3 * math.max(560 / (pos[3]-target_platform_height), maxx - minx)
                    end
                    if(miny < 38) then
                        miny = maxy - 3 * math.max(560 / (pos[3]-target_platform_height), maxx - minx)
                    end        
                end
                print("dasfasd", maxx, minx,maxy,miny)
                ...
                sim.setObjectPosition(targetObj5, d, {0.09, -0.07, -pos[3]+0.28}) 
                ...
                if(ctrl_flag~=3 and ctrl2_cnt < -6)then 
                    target_pos[1] = wayPoint[path_ctrl_flag + 1][1]
                    target_pos[2] = wayPoint[path_ctrl_flag + 1][2]
                    ctrl_flag = 1
                end
        ...
        if(ctrl_flag ~= 2 and ctrl_flag_last_time < 0) then                             
            if(ctrl_flag==3)then
                ...
                if(pos[3] <0.447) then ctrl_flag=4 end
   ...
   elseif(ctrl_flag==7) then
      ...
      --  pos_to_d = sim.multiplyVector(m,linearSpeed_resolute)
        ...
        if(ctrl7_cnt == 0)then
            ...
            --if arrived
            if(pos[3]<0.7 and math.abs(pos_to_door[1] - entry_pos_to_door[1]) < 1
                and((door_pos[3]>0.9 and math.abs(pos_to_door[2] - entry_pos_to_door[2]) < maxDistX)
                or (door_pos[3]<0.5 and math.abs(pos_to_door[3] - entry_pos_to_door[3]) < maxDistX))
